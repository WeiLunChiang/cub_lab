def create_json():
    ...
    data_metadata = pd.read_csv(CSV_FILE)
    test_sql = data_metadata["變形問題"].values
    test_sql_category = data_metadata["編號"].values
...
def summary_json():
    ...
    # %%
    r = []
    for cate in list("ABCD"):
        d = [ix for ix in data if ix["category"] == cate]
